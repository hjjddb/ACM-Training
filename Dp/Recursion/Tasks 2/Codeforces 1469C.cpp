...
ii join(const ii &x, const ii &y){
    return make_pair(max(x.fi, y.fi), min(x.se, y.se));
}
...
int main(){
    ...
    while(t--){
        ...
        vii dp(n);
        dp[0]={a[0], a[0]};
        FOR(i, 1, n-1) dp[i] = join(dp[i-1]+make_pair(-k+1, k-1), make_pair(a[i], a[i]+k-1));
        dp[n-1] = join(dp[n-2]+make_pair(-k+1, k-1), make_pair(a[n-1], a[n-1]));
        bool flag(1);
        FOR(n) flag &= dp[i].se>=dp[i].fi;
        cout << (flag? "YES" : "NO") << '\n';
    }
}