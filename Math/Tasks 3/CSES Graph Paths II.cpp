...
struct Mat {
    ...
    Mat(int nrow, int ncol){
        this->nrow = nrow;
        this->ncol = ncol;
        this->c = vt<vt<ll>>(nrow+1, vt<ll>(ncol+1, oo));
    }
...
Mat operator * (Mat x, Mat y){
    ...
    FOR(i, 1, res.nrow+1)
        FOR(j, 1, res.ncol+1)
            FOR(k, 1, x.ncol+1) res.c[i][j] = min(res.c[i][j], x.c[i][k]+y.c[k][j]);
    return res;
}
...
int main(){
    ...
    while(m--){
        ...
        ll w;
        cin >> u >> v >> w;
        x.c[u][v] = min(x.c[u][v], (ll)w);
    }
    Mat powX = powM(x, k);
    if (powX.c[1][n] == oo) cout << -1;
    else cout << powX.c[1][n];
}